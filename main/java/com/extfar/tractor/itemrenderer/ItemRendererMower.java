...
public class ItemRendererMower implements IItemRenderer
{
    protected EntityMower Mower;
    protected ModelBase modelMower;

    protected static RenderItem drawItems = new RenderItem();

    protected ResourceLocation texture;

    public ItemRendererMower(EntityMower Mower, ModelBase model, ResourceLocation texture)
    {
        this.Mower = Mower;
        this.modelMower = model;
        this.texture = texture;
    }

    protected void renderMower(ItemRenderType type, RenderBlocks render, ItemStack item, float translateX, float translateY, float translateZ)
    {
        GL11.glPushMatrix();
        this.transform(item, type);
        FMLClientHandler.instance().getClient().renderEngine.bindTexture(this.texture);
        this.modelMower.render(this.Mower, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
        GL11.glPopMatrix();
    }
    ...
    public void transform(ItemStack itemstack, ItemRenderType type)
    {
        ...
        long var10 = this.Mower.getEntityId() * 493286711L;
        ...
        if (type == ItemRenderType.EQUIPPED)
        {
            ...
            GL11.glTranslatef(0.1F, 0.7F, 0);
            GL11.glRotatef(190, 0F, 1F, 0F);

            if (player != null && player.ridingEntity != null && player.ridingEntity instanceof EntityMower)
            {
...
/*
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data)
    {
        ...
        {
            case EQUIPPED:
                this.renderMower(type, (RenderBlocks) data[0], item, -0.5f, -0.5f, -0.5f);
                break;
