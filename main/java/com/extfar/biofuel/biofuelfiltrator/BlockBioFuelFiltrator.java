...
public class BlockBioFuelFiltrator extends BlockContainer
{

	private static boolean keepInventory = false;

	/** True if this is an active BioFuelFiltrator, false if idle */
	...
	/**
	 * Is the random generator used by BioFuelFiltrator to drop the inventory
	 * contents in random directions.
	 */
	private Random goldRand;

	public BlockBioFuelFiltrator(Material material, boolean active)
	{
		super(material);
		goldRand = new Random();
	...
	{
		return new TileEntityBioFuelFiltrator();
	}
		...
		if (Meta == 0)
		{
			switch (side)
			{
				...
				case 2:
					return theIcon2;
				case 3:
					if (((TileEntityBioFuelFiltrator) tile).isActive())
					{
						return theIcon4;
					}
					else
	...
	{
		TileEntityBioFuelFiltrator gold = (TileEntityBioFuelFiltrator) world
				.getTileEntity(x, y, z);
		if (world.isRemote)
		{
		...
		else
		{
			...
			{
				player.openGui(ExtendedFarming.modid, 3, world, x, y, z);
			}
			return true;
		}
			...
			int p_149749_4_, Block p_149749_5_, int p_149749_6_)
	{
		if (!field_149934_M)
		{
			TileEntityBioFuelFiltrator tileentityBioFuelFiltrator = (TileEntityBioFuelFiltrator) p_149749_1_
					.getTileEntity(p_149749_2_, p_149749_3_, p_149749_4_);

			if (tileentityBioFuelFiltrator != null)
			{
				for (int i1 = 0; i1 < tileentityBioFuelFiltrator.getSizeInventory(); ++i1)
				{
					ItemStack itemstack = tileentityBioFuelFiltrator
							.getStackInSlot(i1);
	 ...
	 * par5, int par6) { if (!keepInventory) { TileEntityBioFuelFiltrator gold =
	 * (TileEntityBioFuelFiltrator)par1World.getBlockTileEntity(par2, par3, par4);
	 ...
	 * this.goldRand.nextFloat() * 0.8F + 0.1F; float var12 =
	 * this.goldRand.nextFloat() * 0.8F + 0.1F; while (item.stackSize > 0) { int
	 * var13 = this.goldRand.nextInt(21) + 10; if (var13 > item.stackSize) {
	 * var13 = item.stackSize; } item.stackSize -= var13; EntityItem var14 = new
	 * EntityItem(par1World, (double)((float)par2 + var10), (double)((float)par3
	 * + var11), (double)((float)par4 + var12), new ItemStack(item.itemID,
	 * var13, item.getItemDamage())); if (item.hasTagCompound()) {
	 * var14.getEntityItem
	 * ().setTagCompound((NBTTagCompound)item.getTagCompound().copy()); } float
	 * var15 = 0.05F; var14.motionX =
	 * (double)((float)this.goldRand.nextGaussian() * var15); var14.motionY =
	 * (double)((float)this.goldRand.nextGaussian() * var15 + 0.2F);
